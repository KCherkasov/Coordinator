...
class CharacterArchetype {
  public:
    CharacterArchetype(const CharacterArchetypeTemplate& table): _own_id(table._own_id), _name(table._name), _description(table._description), _power_mods(table._power_mods), _defense_mods(table._defense_mods) { if (_own_id == FREE_ID) { _own_id = ++_id; } else { if (_own_id > _id) { _id = _own_id; } } }
    ~CharacterArchetype() {}
